...
router.get('/address/:address/counterparties', sendArkham(req => `/counterparties/address/${encodeURIComponent(req.params.address)}`, 'ARCOX_INTEL_PRICE_COUNTERPARTIES', '0.02'))
router.get('/address/:address/flows', sendArkham(req => `/flow/address/${encodeURIComponent(req.params.address)}`, 'ARCOX_INTEL_PRICE_FLOWS', '0.03'))
router.get('/address/:address/history', sendArkham(req => `/history/address/${encodeURIComponent(req.params.address)}`, 'ARCOX_INTEL_PRICE_HISTORY', '0.03'))
router.get('/address/:address/volume', sendArkham(req => `/volume/address/${encodeURIComponent(req.params.address)}`, 'ARCOX_INTEL_PRICE_VOLUME', '0.03'))
router.get('/address/:address/portfolio', sendArkham(req => `/portfolio/address/${encodeURIComponent(req.params.address)}`, 'ARCOX_INTEL_PRICE_BALANCES', '0.01'))
router.get('/tx/:hash', sendArkham(req => `/tx/${encodeURIComponent(req.params.hash)}`, 'ARCOX_INTEL_PRICE_TX', '0.005'))
router.get('/tx/:hash/transfers', sendArkham(req => `/transfers/tx/${encodeURIComponent(req.params.hash)}`, 'ARCOX_INTEL_PRICE_TX', '0.005'))
...
router.get('/entity/:entity/flows', sendArkham(req => `/flow/entity/${encodeURIComponent(req.params.entity)}`, 'ARCOX_INTEL_PRICE_ENTITY', '0.02'))
...
router.get('/token/:id/top-flow', sendArkham(req => `/token/top_flow/${encodeURIComponent(req.params.id)}`, 'ARCOX_INTEL_PRICE_TOKEN_HOLDERS', '0.03'))
