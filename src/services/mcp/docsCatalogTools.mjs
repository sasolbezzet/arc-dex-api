  ...
  registerTool('arcox_execution_guide', 'Return exact step-by-step tool routes for every ARCOX MCP flow so agents do not guess tool order.', {
    ...
    const guide = {
      ...
      flows: [
        ...
        { intent: 'intel', steps: ['arcox_intel_get_* to get invoice', 'arcox_x402_pay_invoice without confirmed for preview', 'user yes', 'pay with confirmed=true', 'retry the same Intel tool with paymentId'] },
