...
const DEFAULT_ALLOWED_ORIGINS = [
  'http://localhost:5173',
  'http://localhost:4173',
  // Vercel is the only public browser/webhook origin. The VPS remains an internal upstream.
...
app.post('/api/auth/passkey-options', apiLimiter, async (req, res) => {
  try {
    ...
    if (agentKey && mode === 'Login') {
      ...
      // No pre-bound credential yet (agent created via connection token or
      // OAuth): fall back to a discoverable login instead of blocking with
...
app.post('/api/session/authorization-attempt', apiLimiter, requireAuth, async (req, res) => {
  try {
    ...
    if (replacingDifferentHash) {
      ...
      // Replacing a previous authorization hash is only safe when that old
      // operation is provably gone: 'failed' (reverted) or 'absent' (no
      // receipt AND no indexed operation — pruned or never accepted). A
      // 'success' or 'pending' hash is still live on the bundler and must not
      // be replaced, or a second addOwners could create a duplicate owner.
      if (previousOutcome !== 'failed' && previousOutcome !== 'absent') {
...
async function quoteCirBtcAmmRoute(tokenIn, tokenOut, amount) {
  ...
  const amountOutDecimal = unitsToDecimal(amountOut, TOKEN_DECIMALS[tokenOut])
  return {
    available: true,
...
app.post('/api/rpc/arc', apiLimiter, async (req, res) => {
  try {
    ...
    // Try RPCs in order: public Arc → dRPC (with auth) → canteenapp fallback.
    let lastError = null
    for (let rpcIndex = 0; rpcIndex < ARC_RPC_URLS.length; rpcIndex++) {
      const rpcUrl = ARC_RPC_URLS[rpcIndex]
      const maxRetries = rpcIndex === 0 ? 2 : 1
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          const upstream = await drpcFetch(rpcUrl, {
            method: 'POST',
          ...
          if (isRateLimit) break // try next RPC
    ...
    console.error('[arc-rpc]', lastError?.message || 'All Arc RPC endpoints failed')
    ...
    for (const rpcUrl of ARC_RPC_URLS) {
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          ...
          if (!resp.ok) {
            if (resp.status === 429) { lastRpcError = `RPC HTTP ${resp.status}`; break }
...
app.post('/api/mint-cctp-from-solana', apiLimiter, requireServerSignedMintAuth, async (req, res) => {
  try {
    ...
    const pc = createPublicClient({ chain: arcTestnet, transport: arcHttpTransports() })
