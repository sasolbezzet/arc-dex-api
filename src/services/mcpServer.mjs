...
export function createMcpServer(userId, context = {}) {
  ...
  // ── ON-CHAIN AGENTIC JOBS (ERC-8004 identity + ERC-8183 commerce via ARC Memo) ──
  // Ported from the frontend Agentic panel so Claude can register the Agent
  // Wallet as an agent identity, create jobs, fund escrow, submit deliverables,
  // and complete jobs — all executed by the MSCA session key.
  ...
  registerTool('arcox_agentic_create_job', 'Create an on-chain ERC-8183 Agent Job via ARC Memo from the Agent Wallet MSCA. Requires provider, evaluator, description. Returns jobId. Requires confirmation.', {
    ...
    try {
      ...
      const info = await resolveActiveMsca(userId, boundMscaWalletAddress)
      if (!info) return { content: [{ type: 'text', text: jsonText(mscaRequiredResult()) }] }
      const { createJobCall, parseJobIdFromLogs, executeMemoViaDelegate } = await import('./agenticJobsService.mjs')
      ...
      const delegate = getAddress(info.delegateAddress || (await import('./agenticJobsService.mjs')).resolveDelegateAddress(info.walletAddress))
  ...
  registerTool('arcox_agentic_ask', 'Ask the hosted ARCOX agent for a plan/simulation mapped to the Agentic jobs workflow (read-only; no funds move).', {
    ...
    try {
      const info = await resolveActiveMsca(userId, boundMscaWalletAddress)
      if (!info) return { content: [{ type: 'text', text: jsonText(mscaRequiredResult()) }] }
      const r = await fetch(`${BACKEND_URL}/api/agent/ask`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(mintOwnerToken(userId) ? { Authorization: `Bearer ${mintOwnerToken(userId)}` } : {}) },
